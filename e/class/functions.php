...
function getTags($title,$content){
    $subjectenc = rawurlencode(esub(strip_tags($title),60));
    $messageenc = rawurlencode(esub(strip_tags(preg_replace("/\[.+?\]/U", '',$content)),1200));
    ...
    if($data) {
    	...
    	foreach($values as $valuearray) {
    		if($valuearray['tag'] == 'kw' || $valuearray['tag'] == 'ekw') {
    		  ...
    		  if(!in_array($valuearray['value'],array('主题班会','班会')))
